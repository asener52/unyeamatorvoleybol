...
const sections = [
  { label: 'Haberler', col: 'news', icon: FaNewspaper, to: '/admin/haberler', color: 'bg-blue-500', lightColor: 'bg-blue-50 text-blue-700' },
  { label: 'Slider', col: 'sliders', icon: FaPhotoVideo, to: '/admin/slider', color: 'bg-indigo-500', lightColor: 'bg-indigo-50 text-indigo-700' },
  { label: 'Galeri', col: 'gallery', icon: FaImages, to: '/admin/galeri', color: 'bg-purple-500', lightColor: 'bg-purple-50 text-purple-700' },
  { label: 'Etkinlikler', col: 'events', icon: FaCalendarAlt, to: '/admin/etkinlikler', color: 'bg-green-500', lightColor: 'bg-green-50 text-green-700' },
  { label: 'Anketler', col: 'polls', icon: FaPoll, to: '/admin/anketler', color: 'bg-orange-500', lightColor: 'bg-orange-50 text-orange-700' },
]
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
        {sections.map(({ label, col, icon: Icon, to, color, lightColor }) => (
          <Link
            key={col}
            to={to}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {sections.map(({ label, icon: Icon, to, lightColor }) => (
          <Link
            key={label}
            to={`${to}?new=1`}
